...
8. run `npm install` from the root
9. cd into `client` and run `npm i`
10. go back to the root dir and run `npm start` to run the frontend or `npm run full-stack` to run both the front and backend
